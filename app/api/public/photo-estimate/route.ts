...
import { llamaVisionJson } from '../../../../lib/llama'
...
async function visionAddress(image: string) {
  try {
    const { text, model } = await llamaVisionJson(
      image,
      'House photo, often MLS. JSON only {"address":"street city state zip"} or {"address":""}. Do not invent a street number.',
    )
    const match = text.match(/\{[\s\S]*\}/)
    if (!match) return { address: '', note: `${model}: ${text.slice(0, 160)}` }
    const parsed = JSON.parse(match[0])
    return { address: String(parsed.address || ''), note: `llama:${model}` }
  } catch (err) {
    return { address: '', note: err instanceof Error ? err.message : 'llama failed' }
  }
}
