...
export async function POST() {
  ...
  for (const item of watchlist ?? []) {
    ...
    const response = await fetch(url, { headers: { 'x-rapidapi-host': HOST, 'x-rapidapi-key': rapidApiKey }, cache: 'no-store' })
    const text = await response.text()
    let payload: unknown; try { payload = JSON.parse(text) } catch { payload = { message: text.slice(0, 500) } }
