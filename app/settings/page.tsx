...
export default function SettingsPage() {
  ...
  const refreshPrices = async () => {
    setRefreshing(true); setMessage('Refreshing the active Home Depot and Lowe’s watchlist…')
  ...
  return <div className="min-h-screen bg-gray-50 pb-20">
    ...
    <main className="p-4 max-w-2xl mx-auto space-y-4">
      <section className="bg-white rounded-lg shadow p-4"><h2 className="font-semibold mb-1">Pricing refresh</h2><p className="text-xs text-gray-500 mb-3">Weekly is the default. Manual refresh uses the approved Home Depot and Lowe’s retailer watchlist and the 100-inquiry monthly budget per provider.</p><label className="block text-sm">Update frequency<select value={settings.price_refresh_frequency} onChange={e => setSettings({ ...settings, price_refresh_frequency: e.target.value })} className="w-full mt-1 p-2 border rounded"><option value="weekly">Weekly</option><option value="manual">Manual only</option><option value="disabled">Disabled</option></select></label><button onClick={refreshPrices} disabled={refreshing || settings.price_refresh_frequency === 'disabled'} className="mt-3 bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-50">{refreshing ? 'Refreshing…' : 'Refresh prices now'}</button></section>
