...
export default function TasksPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <header className="bg-blue-600 text-white shadow-lg sticky top-0 z-10"><div className="px-4 py-3 flex items-center"><button onClick={() => router.back()} className="text-white mr-3 text-xl">←</button><h1 className="text-xl font-bold">Tasks</h1></div></header>
      <main className="p-4">
        <form onSubmit={addTask} className="bg-white rounded-lg shadow p-4 mb-4 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <select value={form.leadId} onChange={(e) => setForm({ ...form, leadId: e.target.value })} className="flex-1 p-2 border rounded text-sm"><option value="">No lead</option>{leads.map((lead) => <option key={lead.id} value={lead.id}>{lead.name}</option>)}</select>
          </div>
        ...
        {loading ? <p className="text-sm text-gray-500">Loading tasks…</p> : tasks.map((task) => (
          <div key={task.id} className="bg-white rounded-lg shadow p-4 mb-3 flex gap-3">
            <button onClick={() => void toggleTask(task)} className={`mt-1 w-5 h-5 rounded border-2 ${task.status === 'completed' ? 'bg-green-500 border-green-500' : 'border-gray-300'}`} />
            <div><p className={task.status === 'completed' ? 'line-through text-gray-400' : 'font-medium'}>{task.title}</p>{task.lead_id && <button onClick={() => router.push(`/leads/${task.lead_id}`)} className="text-xs text-blue-600">Open lead</button>}</div>
          </div>
        ))}
      </main>
    </div>
