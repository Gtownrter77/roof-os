...
type Appointment = { id: string; title: string; starts_at: string; status: string }
...
export default function LeadDetailPage() {
  ...
  async function load() {
    ...
    const [{ data: leadRow, error: leadError }, activityRes, apptRes, taskRes] = await Promise.all([
      ...
      supabase.from('appointments').select('id,title,starts_at,status').eq('lead_id', leadId).order('starts_at', { ascending: false }).limit(20),
      supabase.from('tasks').select('id,title,status,due_at').eq('lead_id', leadId).order('due_at', { ascending: true, nullsFirst: false }).limit(20),
    ])
  ...
  async function workspaceAndUser() {
    const { data: { user } } = await supabase.auth.getUser()
  ...
  async function updateStatus(status: string) {
    ...
    const { user, workspaceId } = await workspaceAndUser()
  ...
  async function addNote() {
    ...
    const { user, workspaceId } = await workspaceAndUser()
    if (!user || !workspaceId) { setError('No workspace available.'); setSaving(false); return }
    const { error: insertError } = await supabase.from('lead_activity').insert({ lead_id: lead.id, workspace_id: workspaceId, user_id: user.id, kind: 'note', body })
  ...
  async function scheduleInspection() {
    ...
    await supabase.from('lead_activity').insert({ lead_id: lead.id, workspace_id: workspaceId, user_id: user.id, kind: 'appointment', body: `Inspection scheduled for ${start.toLocaleString()}` })
    setApptAt('')
    await load()
    setSaving(false)
  }
  ...
  async function startInspection() {
    ...
    const { user, workspaceId } = await workspaceAndUser()
    if (!user || !workspaceId) { setError('No workspace available.'); setSaving(false); return }
    const { data, error: insertError } = await supabase.from('inspection_sessions').insert({
      workspace_id: workspaceId,
      lead_id: lead.id,
      created_by: user.id,
      status: 'in_progress',
    ...
    if (insertError) { setError(insertError.message); setSaving(false); return }
    await supabase.from('leads').update({ status: 'inspected', updated_at: new Date().toISOString() }).eq('id', lead.id)
      ...
      {lead && (
        <>
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-4">
            ...
            {appointments.map((item) => <p key={item.id} className="text-sm border-b py-2">{item.title} · {new Date(item.starts_at).toLocaleString()}</p>)}
          </div>
          <div className="bg-white rounded-lg shadow p-4 mb-4">
            <h2 className="font-semibold mb-2">Tasks</h2>
            {tasks.map((item) => <p key={item.id} className="text-sm border-b py-2">{item.title} · {item.status}</p>)}
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            ...
            {activity.map((item) => (
              <div key={item.id} className="text-xs bg-gray-50 rounded p-2 mb-2">
                <span className="font-semibold">{item.kind.replace('_', ' ')}</span> · {item.body}
              </div>
            ))}
          </div>
        </>
