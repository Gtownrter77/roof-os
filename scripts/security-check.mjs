...
const providerCall = Math.max(capout.indexOf('fetchWithTimeout(`${CAPOUT_API}/upload`'), capout.indexOf('fetch(`${CAPOUT_API}/upload`'))
assert.ok(providerCall > -1)
assert.ok(capout.indexOf("from('workspace_members')") < providerCall)
assert.ok(capout.indexOf("from('claims_imports').insert") < providerCall)
...
assert.ok(fieldApp.includes('VALUES (?, ?, ?, ?, ?, ?)'))
