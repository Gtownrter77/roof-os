...
export default function App() {
  ...
  useEffect(() => {
    ...
    const consumeMagicLink = async (url: string | null) => {
      if (!url || !supabase) return
      const code = new URL(url).searchParams.get('code')
      if (code) await supabase.auth.exchangeCodeForSession(code)
    }
    Linking.getInitialURL().then(consumeMagicLink)
    const linkSubscription = Linking.addEventListener('url', ({ url }) => { void consumeMagicLink(url) })
  ...
  async function sendMagicLink() {
    ...
    if (!email.trim()) { setError('Enter your work email to receive a secure sign-in link.'); return }
    setAuthBusy(true); setError('')
    const { error: authError } = await supabase.auth.signInWithOtp({ email: email.trim(), options: { emailRedirectTo: Linking.createURL('auth/callback') } })
    if (authError) setError(authError.message)
    else setNotice('Check your email for the secure ROOF/OS sign-in link, then return to the app.')
    setAuthBusy(false)
  }
  ...
  if (!session) return <AuthScreen email={email} setEmail={setEmail} sendMagicLink={sendMagicLink} busy={authBusy} error={error} notice={notice} configured={Boolean(supabase)}/>
...
function AuthScreen({ email, setEmail, sendMagicLink, busy, error, notice, configured }: { email: string; setEmail: (value: string) => void; sendMagicLink: () => void; busy: boolean; error: string; notice: string; configured: boolean }) { return <SafeAreaView style={styles.center}><View style={styles.authCard}><Text style={styles.eyebrow}>ROOF/OS FIELD</Text><Text style={styles.authTitle}>Secure field access</Text><Text style={styles.helper}>Enter your work email and we’ll send a passwordless sign-in link.</Text><TextInput value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" placeholder="Work email" placeholderTextColor="#8b99aa" style={styles.input}/><Pressable style={styles.primary} onPress={sendMagicLink} disabled={busy}><Text style={styles.primaryText}>{busy ? 'Sending link…' : 'Email me a sign-in link'}</Text></Pressable>{notice ? <Text style={styles.notice}>{notice}</Text> : null}{!configured ? <Text style={styles.error}>Mobile Supabase environment variables are not configured.</Text> : null}{error ? <Text style={styles.error}>{error}</Text> : null}</View></SafeAreaView> }
