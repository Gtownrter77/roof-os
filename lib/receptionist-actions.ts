...
export async function bookAppointment(input: { workspaceId: string; leadId: string; startsAt: string; title: string; address?: string; idempotencyKey: string }) {
  ...
  const { data: conflicts, error: conflictError } = await supabase.from('appointments').select('id').eq('workspace_id', input.workspaceId).lt('starts_at', end.toISOString()).gt('ends_at', start.toISOString()).limit(10)
